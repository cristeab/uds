...
int main(void)
{
	...
	while (1) {
		/********************** Get version of server ***********************/
		{
			...
			ver = (uds_response_version_t *)client_send_request(
				clnt, &req);
			if (ver == NULL) {
			...
			if (ver->common.status == STATUS_SUCCESS) {
				if (ver->common.data_len !=
				    (sizeof(uds_response_version_t) -
				     sizeof(uds_command_t))) {
		...
		/********************** Get message from server ***********************/
		{
			...
			res = (uds_response_get_msg_t *)client_send_request(
				clnt, &req);
			if (res == NULL) {
				printf("client: send request error\n");
				client_close(clnt);
		...
		/********************** Put message to server ***********************/
		{
			...
			snprintf((char *)req.data, UDS_PUT_MSG_SIZE - 1, "%s",
				 str);
			req.data[UDS_PUT_MSG_SIZE - 1] = 0;

			res = (uds_command_t *)client_send_request(
				clnt, (uds_command_t *)&req);
			if (res == NULL) {
				printf("client: send request error\n");
				client_close(clnt);
				return STATUS_ERROR;
